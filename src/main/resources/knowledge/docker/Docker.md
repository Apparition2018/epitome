...
1. [容器化应用程序](https://docs.docker.com/get-started/02_our_app/)
    ...
    2. Create Dockerfile：`cd /getting-started-app` → `vim Dockerfile`
        ...
        EXPOSE 3000
        ```
    3. Build image：`docker build -t getting-started .`
    4. Run container：`docker run -dp 127.0.0.1:3000:3000 getting-started` → http://localhost:3000
...
|-----------|--------------------------|----------------------------------------------------|
| Container | `docker container prune` | `-f` `--force` `--filter`                          |
| Image     | `docker image prune`     | `-f` `--force` `--filter` `-a` `--all`             |
| Network   | `docker network prune`   | `-f` `--force` `--filter`                          |
| Volume    | `docker volume prune`    | `-f` `--force` `--filter`                          |
| System    | `docker system prune`    | `-f` `--force` `--filter` `-a` `--all` `--volumes` |
---
...
- 配置文件的查找顺序：`mysql --verbose --help | grep -A 1 'Default options'`
```bash
...
- [redis.conf](https://redis.io/docs/latest/operate/oss_and_stack/management/config/) 选择对应版本
    - redis.conf
    ```
    requirepass 123456
    bind 0.0.0.0
    ...
    ```
```bash
mkdir -p /home/lighthouse/docker_data/redis/{data,conf}
...
-v $PWD/conf:/usr/local/etc/redis/ \
redis redis-server /usr/local/etc/redis/redis-replica.conf
```
```bash
...
```
---
## [RocketMQ](https://hub.docker.com/r/apache/rocketmq)
...
```
---
## [EMQX](https://hub.docker.com/r/emqx/emqx-enterprise)
```bash
mkdir -p /home/ljh/docker_data/emqx/{data,log,etc}
cd /home/ljh/docker_data/emqx
```
```bash
