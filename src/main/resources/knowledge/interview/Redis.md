...
2. 击穿 (Breakdown)：热点缓存在过期的瞬间有大量请求
    ...
    - 不设置过期时间：
        1. 将过期时间写在 value 里，代码判断是否过期异步更新
        2. 定时更新（慎用）
3. 雪崩 (Avalanche)
    1. 大量缓存同时过期
        - 参考击穿
...
| List        | 简单队列、最近/最新、时间线、定时排行榜                                        |
| Set         | 唯一数据（标签、关注、点赞）、集合操作（共同好友、商品筛选）、抽奖（SPOP）                     |
| Sorted Set  | 排序（实时排行榜）、延迟队列、带权重的优先级队列                                    |
| Hash        | 对象存储（用户信息、购物车）、条件查询（Lua）                                    |
...
1. RDB：Redis Database，指定时间间隔执行的数据集快照
    - 优点：
        1. 二进制快照，恢复极快
        2. 在 Redis 复制，RDB 支持重启和故障转移后的部分重新同步
    - 缺点：可能丢失两次备份间的数据（快达修改阈值时断电宕机）
    - redis.conf
        ```
        # RDB 持久化文件名
        dbfilename dump.rdb
...
2. AOF：Append Only File，记录每个写入操作
    - 优点：
        1. 由于同步策略：`always`（每个命令）、`everysec`（每秒），所以使用 AOF 最多丢失一秒的写入
