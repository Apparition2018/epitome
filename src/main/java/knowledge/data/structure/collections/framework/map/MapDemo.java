 ...
 * <p>************************************************************
 * <pre>
 * 特性	        普通二叉搜索树 (BST)	                AVL 树	                                红黑树
 * 平衡条件	        无要求	                左右子树高度差 ≤ 1（严格）	            黑高一致，最长路径 ≤ 2×最短路径（宽松）
 * 高度	        最坏 O(n)（退化成链表）	        严格 O(log n)	                    O(log n)，常数略大于 AVL
 * 查找性能	    平均 O(log n)，最坏 O(n)	        最快，树最紧凑	                    较快，稍慢于 AVL
 * 插入性能	    平均 O(log n)，最坏 O(n)	    较慢，可能需要多次旋转	                快，最多 2 次旋转 + 变色
 * 删除性能	    平均 O(log n)，最坏 O(n)	    慢，可能引发多次旋转	                快，最多 3 次旋转 + 变色
 * 旋转/调整次数	    0	                    较多（尤其删除时）                       很少，以变色为主，旋转次数固定
 * 实现复杂度	    简单	                    中等（需维护平衡因子）	                较复杂（需处理多种颜色规则）
 * 内存开销	    仅存 key/value	            额外存平衡因子/高度	                额外存颜色信息（通常 1 bit）
 * 适用场景	    极少使用，教学为主	            读多写少场景（如搜索索引）	            通用读写场景，工业界首选
 * </pre>
 *
 * @author ljh
