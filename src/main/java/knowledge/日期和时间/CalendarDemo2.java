...
public class CalendarDemo2 {
    ...
    public void testAddAndSet() {
        ...
        System.out.println(calendar.getTime());         // Mon Oct 10 20:08:08 CST 2011
    }
    ...
    public void testGet() {
        ...
        System.out.println(year + "-" + month + "-" + day); // 2008-8-8

        int h = calendar.get(Calendar.HOUR);                // 12小时制
        h = calendar.get(Calendar.HOUR_OF_DAY);             // 24小时制
        int m = calendar.get(Calendar.MINUTE);
        int s = calendar.get(Calendar.SECOND);
        System.out.println(h + ":" + m + ":" + s);          // 20:8:8

        int doy = calendar.get(Calendar.DAY_OF_YEAR);
        System.out.println("当年的第" + doy + "天");         // 当年的第221天

        int dow = calendar.get(Calendar.DAY_OF_WEEK) - 1;   // 从星期日开始，且常量为1
        String[] data = {"日", "一", "二", "三", "四", "五", "六"};
        System.out.println("当年的星期" + data[dow]);        // 当年的星期五

        int max = calendar.getActualMaximum(Calendar.DAY_OF_MONTH);
        System.out.println("当月共" + max + "天");          // 当月共31天
        max = calendar.getActualMaximum(Calendar.DAY_OF_YEAR);
        System.out.println("当年共" + max + "天");          // 当年共366天
    }
