 ...
 * </pre>
 * Spring 应用程序事件（Application Events）由三部分组成：
 * <pre>
 * 1 Event：事件，ApplicationEvent，继承自 JDK 的 EventObject，所有事件将继承它，并通过 source 得到事件源。
 * 2 Publisher：发布者，ApplicationEventPublisher 及 ApplicationEventMulticaster 接口，使用这个接口，我们的 Service 就拥有了发布事件的能力。
 * 3 Listener：监听器
 *  ① ApplicationListener，继承自 JDK 的 EventListener，所有监听器将继承它
 *  ② @EventListener
 * </pre>
...
public class SpringApplicationEventDemo {
    ...
    /**
     * <pre>
     *  Pattern                 `@Test          OrderEvent      Listener
     *  Delegation Event Model  Event Source    Event Object    Event Listener
     *  Application Events      Publisher       Event   `       Listener
     * </pre>
     */
    @Test
    public void testSpringListener() {
        // AnnotationConfigApplicationContext 是 ApplicationEventPublisher 的具体实现类
        // 如果是在 Spring 项目，应该通过注入 ApplicationEventPublisher 来使用该功能
        // Publisher（发布者）
...
class SmsListener implements ApplicationListener<OrderEvent> {
...
class EmailListener implements ApplicationListener<OrderEvent> {
